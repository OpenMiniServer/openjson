...
const char* OpenJson::data()
{
    ...
    PRINTF("OpenJson warn:JsonNode is Empty\n");
...
const std::string& OpenJson::s()
{
    ...
    {
        PRINTF("OpenJson warn:JsonNode is no STRING\n");
        if (!segment_)
        {
...
double OpenJson::d(double def)
{
    if (type_ != NUMBER) 
    {
        PRINTF("OpenJson warn:JsonNode is no NUMBER\n");
        return def;
    }
    if (segment_ == 0)
    {
    ...
    if (segment_->type_ != Segment::DOUBLE)
    {
        if (!context_ || !context_->data_ || len_ < 1)
        {
            PRINTF("OpenJson warn:JsonNode is no DOUBLE NUMBER");
        }
        else
        {
...
bool OpenJson::b(bool def)
{
    ...
    if (segment_->type_ != Segment::BOOL)
    {
        if (!context_ || !context_->data_ || len_ < 1)
        {
            PRINTF("OpenJson warn:JsonNode is no BOOL NUMBER");
        }
        else
        {
...
int32_t OpenJson::i32(int32_t def)
{
    ...
    if (segment_->type_ != Segment::INT32)
    {
        if (!context_ || !context_->data_ || len_ < 1)
        {
            PRINTF("OpenJson warn:JsonNode is no INT32 NUMBER");
        }
        else
        {
...
int64_t OpenJson::i64(int64_t def)
{
    ...
    {
        PRINTF("OpenJson warn:JsonNode is no INT64 NUMBER");
    }
    switch (segment_->type_)
    {
    case OpenJson::Segment::BOOL:
...
void OpenJson::operator=(const std::string& val)
{
    ...
    {
        PRINTF("OpenJson warn:JsonNode is a container");
        return;
    }
...
OpenJson& OpenJson::array(size_t idx)
{
    if (type_ != ARRAY)
    {
        if (type_ == OBJECT)
        {
            PRINTF("WARN:JsonNode must be ARRAY, not OBJECT");
        }
        type_ = ARRAY;
    }
    else
...
OpenJson& OpenJson::object(const char* key)
{
    ...
    if (type_ != OBJECT)
    {
        if (type_ == ARRAY)
        {
            PRINTF("WARN:JsonNode must be OBJECT, not ARRAY");
        }
        type_ = OBJECT;
    }
    else
...
void OpenJson::addNode(OpenJson* node)
{
    ...
    if (type_ != OBJECT && type_ != ARRAY)
    {
        PRINTF("OpenJson warn:JsonNode must be OBJECT or ARRAY");
...
{
    assert(type_ == NUMBER);
    if (segment_) segment_->toString();
    const std::string& val = segment_ ? segment_->content_ : s();
    if (key_)
    {
        wcontext_->wbuffer_.append("\"" + key() + "\":" + val);
    }
    else
    {
        wcontext_->wbuffer_.append(val);
    }
}
void OpenJson::writeString()
{
    ...
    if (key_)
    {
        wcontext_->wbuffer_.append("\"" + key() + "\":\"" + s() + "\"");
    }
    else
    {
...
void OpenJson::throwError(const char* errMsg)
{
    ...
    const char* tab = type_ >= 0 && type_ < len ? InfoTags[type_] : InfoTags[5];
